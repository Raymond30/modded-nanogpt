"""
train_gpt_simple_leonh.py

This file descends from the [NanoGPT speedrun](https://github.com/KellerJordan/modded-nanogpt).
It was prepared as a simplified version of the speedrun for use in neural net optimization research.

Differs from `train_gpt_simple_leon.py` only in the optimizer update rule and init: the
decoupled weight decay step is replaced by a Frobenius-norm-preserving hyperball projection
(scale_invariant_update_). This keeps ||param||_F constant across training, making weight
decay redundant. The residual-side projections / mlp.fc are initialised with per-module
multipliers on the default Kaiming init so LeonH has non-zero matrices to operate on from
step 0 (zeroed weights would give a degenerate hyperball step of size zero).
"""
...
BASE_HPARAMS = dict(
    ...
    # LeonH — 2D block params (no weight_decay: hyperball projection replaces it)
    leon_lr             = 0.025,
    leon_mu             = 0.95,
    leon_beta2          = 0.7,
    leon_cooldown_frac  = 0.7,
    leon_min_lr_frac    = 0.0,
    leon_ns_iters       = 6,
    leon_eps            = 1e-9,
    leon_l_scale        = 1.0,
    leon_precondition_side = "auto",  # "auto" | "input" | "output"
...
def parse_args():
    ...
    parser.add_argument("--out-dir", type=Path, default=None,
                        help="Run output directory. Defaults to runs/leonh/<timestamp>-<uuid>.")
...
def validate_hparams(hparams):
    ...
    side = hparams["leon_precondition_side"]
    if side not in ("auto", "input", "output"):
        raise ValueError(
            "leon_precondition_side must be 'auto', 'input', or 'output', "
            f"got {side!r}"
        )
...
def default_run_dir():
    ...
    return Path("records/track_3_optimization/runs/leonh") / f"{stamp}-{uuid.uuid4().hex[:8]}"
...
def leon_orthogonalize(grad: Tensor, momentum_buffer: Tensor, second_momentum_buffer: Tensor,
                       ...
                       precondition_side: str = "auto") -> Tensor:
    """
    ...
    Args:
        ...
        precondition_side: which gradient side to precondition from.
            "auto"   - smaller of input/output dim (default, current behavior);
            "input"  - always (d_in, d_in) Gram via grad.mT @ grad, multiply X@B on the right
                       (PyTorch weight shape is (d_out, d_in), so d_in = grad.size(-1));
            "output" - always (d_out, d_out) Gram via grad @ grad.mT, multiply B@X on the left.
    """
    if precondition_side == "input":
        is_tall = True
    elif precondition_side == "output":
        is_tall = False
    else:
    ...
    # 4. Initial augmented Gram: A0 = gram(X) + L_scaled
    if is_tall:
        A = A + (X.mT @ X)
    else:
        A = A + (X @ X.mT)
    ...
    for i in range(ns_iters):
        ...
        # Update A alongside X (skip on last iteration)
        if i < last_iter:
            # A <- (aI + B) A (aI + B)
            # computed as: D = a*A + B@A, then A = a*D + D@B
            D = a * A + B @ A
            A = a * D + D @ B
...
def scale_invariant_update_(param: Tensor, update: Tensor, lr: float, eps: float = 1e-10) -> None:
    """Hyperball-constrained step: take a LeonH update of size lr * ||param||_F,
    then renormalise back onto the Frobenius sphere of the parameter's current radius.
    Preserves ||param||_F exactly across training; the invariant lets us drop weight
    decay on hidden matrices entirely (the constraint already prevents norm growth)."""
    p_norm = param.norm()
    u_norm = update.norm()
...
class LeonH(torch.optim.Optimizer):
    """
    LeonH optimizer: Leon's Nesterov momentum + Newton-Schulz orthogonalization with
    augmented Gram matrix, applied via a Frobenius-norm-preserving hyperball projection.

    Weight decay is replaced by the hyperball constraint: ||param||_F is preserved exactly
    at every step, preventing norm growth without explicit regularization.
    """
            ...
            for group in self.param_groups:
                ...
                for base_i in range(0, len(params), world_size):
                    ...
                    if len(state) == 0:
                        ...
                        side = group["precondition_side"]
                        gram_D = (d_in if side == "input"
                                  else d_out if side == "output"
                                  else min(d_out, d_in))
                        second_momentum_buffer = torch.zeros(
                            gram_D, gram_D, dtype=torch.float32, device=p.device
                        )
                    else:
...
def main():
    ...
    try:
        ...
        for _ in range(num_trials):
            ...
            # initialize model parameters. Per-module multipliers on the default nn.Linear
            # Kaiming-uniform init (std = 1/sqrt(3*fan_in)):
            ...
            # Non-zero init is required: the hyperball projection step is proportional to
            # ||param||_F, so zero-initialised matrices would never move.
            ...
            def set_hparams(step):
                ...
                assert 0 <= progress < 1
                min_lr_frac = hparams.get("leon_min_lr_frac", 0.0)
                for opt in optimizers:
                    for group in opt.param_groups:
                        ...
                        if progress < 1 - cooldown_frac:
                            ...
                            decay = (1 - progress) / cooldown_frac
                            if opt is optimizer2 and min_lr_frac > 0.0:
                                eta = min_lr_frac + (1 - min_lr_frac) * decay
                            else:
                                eta = decay
