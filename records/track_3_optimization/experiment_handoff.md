...
Last updated: 2026-05-06 12:47 America/Chicago
...
| Orthogonalization dtype probes | `leon_orth_float32_lr035_wd0275_b260_4gpu`, `leon_orth_float64_lr035_wd0275_b260_eps1e12_4gpu` | Added `leon_orthogonalize_dtype` with default `bfloat16`, then ran the `retune1500_lr035_wd0275_b260_4gpu` config with higher-precision NS computation. `float32` ended at `3.45121`; `float64` plus `leon_eps=1e-12` ended at `3.45027`. Both trailed the bfloat16 reference `3.44937`, so dtype precision alone has not closed the Muon gap. |
...
4. If comparing against Muon, keep both Muon baselines in view: original 12-step Muon is the stronger reference, while Muon Leon-NS isolates part of the orthogonalization change. The float32 and float64 Leon orthogonalization probes did not beat the bfloat16 reference, so dtype precision alone is not supported as the main remaining gap.
