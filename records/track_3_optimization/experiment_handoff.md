...
| Orthogonalization dtype probe | `leon_orth_float32_lr035_wd0275_b260_4gpu` | Added `leon_orthogonalize_dtype` with default `bfloat16`, then ran the `retune1500_lr035_wd0275_b260_4gpu` config with `float32` for `X` and NS matrix updates. Final `3.45121` trailed the bfloat16 reference by `+0.00184`, so this did not close the Muon gap. |
