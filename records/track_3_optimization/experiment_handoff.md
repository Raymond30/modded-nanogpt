...
Last updated: 2026-05-06 13:27 America/Chicago
...
| --- | --- | --- | ---: | --- |
| Best Leon screen | `leon_orth_float64_lr035_wd0275_b260_eps1e12_ns12_4gpu` | `leon_lr=0.035`, `leon_wd=0.0275`, `leon_cooldown_frac=0.6`, `leon_beta2=0.6`, `leon_ns_iters=12`, `leon_orthogonalize_dtype=float64`, `leon_eps=1e-12` | 3.43039 | `records/track_3_optimization/runs/leon/20260506-leon-orth-float64-4de4a57/lr035_wd0275_b260_float64_eps1e12_ns12_4gpu` |
...
- The float64 12-step Newton-Schulz follow-up, `leon_orth_float64_lr035_wd0275_b260_eps1e12_ns12_4gpu`, finished at `3.43039`. That beats the 6-step bf16 reference by `-0.01898`, the 6-step float64 probe by `-0.01988`, the best prior Leon schedule by `-0.01614`, and the original `muon1500_2gpu` baseline by `-0.00040`; it still did not reach `val_loss < 3.28`.
...
- The float64 precision-only probe with `leon_eps=1e-12` and the same `leon_ns_iters=6` finished at `3.45027`, still `+0.00090` worse than the bfloat16 reference. Increasing to `leon_ns_iters=12` at the same float64/eps setting finished at `3.43039`, so iteration count, not dtype precision alone, was the decisive tested factor.
...
| Orthogonalization dtype and NS-step probes | `leon_orth_float32_lr035_wd0275_b260_4gpu`, `leon_orth_float64_lr035_wd0275_b260_eps1e12_4gpu`, `leon_orth_float64_lr035_wd0275_b260_eps1e12_ns12_4gpu` | Added `leon_orthogonalize_dtype` with default `bfloat16`, then ran the `retune1500_lr035_wd0275_b260_4gpu` config with higher-precision NS computation. `float32` ended at `3.45121`; `float64` plus `leon_eps=1e-12` ended at `3.45027`; increasing `leon_ns_iters` from `6` to `12` ended at `3.43039`, slightly beating original Muon. |
...
4. If comparing against Muon, keep both Muon baselines in view: original 12-step Muon is the stronger reference, while Muon Leon-NS isolates part of the orthogonalization change. The float32 and float64 6-step Leon orthogonalization probes did not beat the bfloat16 reference, but the 12-step follow-up did, so the next clean attribution runs should isolate `leon_ns_iters=12` from double precision by trying 12 steps with the cheaper dtype path.
