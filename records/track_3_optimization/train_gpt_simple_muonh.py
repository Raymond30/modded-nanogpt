...
def parse_args():
    ...
    parser.add_argument("--out-dir", type=Path, default=None,
                        help="Run output directory. Defaults to runs/muonh/<timestamp>-<uuid>.")
...
def default_run_dir():
    ...
    return Path("records/track_3_optimization/runs/muonh") / f"{stamp}-{uuid.uuid4().hex[:8]}"
...
for _ in range(num_trials):
    ...
    optimizer2 = MuonH([p for p in model.blocks.parameters() if p.ndim == 2], lr=matrix_lr, mu=mu)
