"""
train_gpt_simple_one_sided_shampoo.py

This file descends from the [NanoGPT speedrun](https://github.com/KellerJordan/modded-nanogpt).
It was prepared as a simplified version of the speedrun for use in neural net optimization research.
This variant replaces Muon with one-sided Shampoo, whose `L^{-1/2} M` update direction is
computed via an `AugmentedNS(M, A) = (M Mᵀ + A)^{-1/2} M` Newton–Schulz subroutine driven by
the residual-Gram recursion (M_t, V_t, λ_t) so that L_t = (1/λ_t) M_t M_tᵀ + V_t holds exactly
and the update reduces to √λ_t · AugmentedNS(M_t, λ_t V_t).
"""
...
BASE_HPARAMS = dict(
    ...
    # One-sided Shampoo — 2D block params
    shampoo_lr             = 0.025,
    shampoo_wd             = 0.025,
    shampoo_beta1          = 0.9,   # M EMA coefficient; β₁² < β₂ keeps λ_t bounded
    shampoo_beta2          = 0.95,  # L EMA coefficient
    shampoo_cooldown_frac  = 0.7,
    shampoo_ns_iters       = 6,
    shampoo_eps            = 1e-9,
    shampoo_orthogonalize_dtype = "bfloat16",
...
def parse_args():
    ...
    parser.add_argument("--out-dir", type=Path, default=None,
                        help="Run output directory. Defaults to runs/one_sided_shampoo/<timestamp>-<uuid>.")
...
def resolve_hparams(overrides):
    ...
    for item in overrides:
        ...
        if key == "cooldown_frac":
            ...
            hparams["shampoo_cooldown_frac"] = value
        hparams[key] = value
    return hparams
...
def validate_hparams(hparams):
    orthogonalize_dtype = hparams["shampoo_orthogonalize_dtype"]
    if orthogonalize_dtype not in ("bfloat16", "float32", "float64"):
        raise ValueError(
            "shampoo_orthogonalize_dtype must be 'bfloat16', 'float32', or 'float64', "
            f"got {orthogonalize_dtype!r}"
        )

    b1 = float(hparams["shampoo_beta1"])
    b2 = float(hparams["shampoo_beta2"])
    if not (0 <= b1 < 1):
        raise ValueError(f"shampoo_beta1 must be in [0, 1), got {b1!r}")
    if not (0 <= b2 < 1):
        raise ValueError(f"shampoo_beta2 must be in [0, 1), got {b2!r}")
    if b1 * b1 >= b2:
        # λ_t recursion is unbounded when β₁² ≥ β₂ (spec §2.4); math is still valid.
        print(
            f"[warn] shampoo_beta1²={b1*b1:.6g} ≥ shampoo_beta2={b2:.6g}: "
            "λ_t recursion is unbounded; consider clamping or lowering beta1.",
            file=sys.stderr,
        )
...
def default_run_dir():
    ...
    return Path("records/track_3_optimization/runs/one_sided_shampoo") / f"{stamp}-{uuid.uuid4().hex[:8]}"
...
def _compute_dtype(dtype_name: str):
    ...
    raise ValueError(f"Unsupported orthogonalization dtype: {dtype_name!r}")


def _accum_dtype(dtype_name: str):
    return torch.float64 if dtype_name == "float64" else torch.float32


def augmented_ns(X: Tensor, A: Tensor, ns_iters: int = 6,
                 ns_coeffs: tuple = (2, -1.5, 0.5),
                 eps: float = 1e-9, compute_dtype: str = "bfloat16") -> Tensor:
    """
    AugmentedNS(M, A): compute (M Mᵀ + A)^{-1/2} M via a coupled Newton–Schulz iteration
    that updates an internal A alongside X.

    A is the smaller-side augmentation (shape min_D × min_D).
    For wide X (m < n): iterates X ← aX + B·X with A ← (aI+B)A(aI+B); B = bA + cA²
        ⇒ (X Xᵀ + A)^{-1/2} X
    For tall X (m ≥ n): iterates X ← aX + X·B with the same A update
        ⇒ X (Xᵀ X + A)^{-1/2}  (equivalent under the transpose-to-smaller-side convention).
    """
    is_tall = X.size(-2) >= X.size(-1)
    x_dtype = _compute_dtype(compute_dtype)
    accum_dtype = _accum_dtype(compute_dtype)

    X = X.to(dtype=x_dtype)
    A = A.to(dtype=accum_dtype)
    A = 0.5 * (A + A.transpose(-2, -1))  # ensure symmetry

    # Scale so that tr(X Xᵀ + A) ≈ 1; the (1 + 2e-2)² cushion keeps the iteration inside
    # the basin of attraction even when our X / A estimate of the trace is slightly off.
    ...
    # Initial augmented Gram: A₀ ← A + gram_smaller_side(X)
    if is_tall:
        A = A + (X.mT @ X)
    else:
        A = A + (X @ X.mT)
    ...
    for i in range(ns_iters):
        ...
        if i < last_iter:
            # A ← (aI + B) A (aI + B) computed as D = aA + B·A then A = aD + D·B
            D = a * A + B @ A
            A = a * D + D @ B
...
class OneSidedShampoo(torch.optim.Optimizer):
    """
    One-sided Shampoo via AugmentedNS.

    Maintains per-param state (M, V, λ) satisfying L_t = (1/λ_t) M_t M_tᵀ + V_t exactly,
    where L_t is the EMA of G_t G_tᵀ. The update direction is

        L_t^{-1/2} M_t = √λ_t · AugmentedNS(M_t, λ_t V_t).

    The state recursion (see one_sided_shampoo_augmentedns.md §2.2) uses the pre-update
    M_{t-1} and λ_{t-1}; the t=1 case (λ₀ = 0) is guarded so the M_{t-1} term — formally
    `√(β₂/(λ₀ λ₁)) · M₀ = ∞ · 0` — is dropped instead of evaluated.
    """
    def __init__(self, params, lr=0.025, weight_decay=0.025,
                 beta1=0.9, beta2=0.95, ns_iters=6, eps=1e-9,
                 orthogonalize_dtype="bfloat16"):
        ...
        for group in self.param_groups:
            ...
            for base_i in range(0, len(params), world_size):
                if base_i + rank < len(params):
                    ...
                    M = state["M"]
                    V = state["V"]
                    lam_prev = float(state["lambda"])
                    lam_t = (b1 * b1 / b2) * lam_prev + (1.0 - b1)**2 / (1.0 - b2)

                    # spec §2.3 guard: at t=1, lam_prev==0 ⇒ N_1 = 0 (drop the ∞·0 term)
                    if lam_prev == 0.0:
                        N = torch.zeros_like(grad)
                    else:
                        cg = math.sqrt((b1 * b1 * lam_prev) / (b2 * lam_t))
                        cm = ((1.0 - b1) / (1.0 - b2)) * math.sqrt(b2 / (lam_prev * lam_t))
                        N = cg * grad - cm * M

                    # update V on the smaller side, then M, then λ (uses M_{t-1}, λ_{t-1} above)
                    is_tall = p.size(-2) >= p.size(-1)
                    NN = (N.mT @ N) if is_tall else (N @ N.mT)
                    V.mul_(b2).add_(NN, alpha=(1.0 - b2))
                    M.mul_(b1).add_(grad, alpha=(1.0 - b1))
                    state["lambda"] = lam_t

                    A = lam_t * V
                    direction = augmented_ns(
                        M, A,
                        ns_iters=group["ns_iters"], eps=group["eps"],
                        compute_dtype=group["orthogonalize_dtype"],
                    )
                    direction *= max(1, p.size(-2) / p.size(-1))**0.5
...
def main():
    ...
    try:
        ...
        for _ in range(num_trials):
            ...
            optimizer2 = OneSidedShampoo([p for p in model.blocks.parameters() if p.ndim >= 2],
                                         lr=hparams["shampoo_lr"], weight_decay=hparams["shampoo_wd"],
                                         beta1=hparams["shampoo_beta1"], beta2=hparams["shampoo_beta2"],
                                         ns_iters=hparams["shampoo_ns_iters"], eps=hparams["shampoo_eps"],
                                         orthogonalize_dtype=hparams["shampoo_orthogonalize_dtype"])
            ...
            for group in optimizer2.param_groups:
                group["cooldown_frac"] = hparams["shampoo_cooldown_frac"]
